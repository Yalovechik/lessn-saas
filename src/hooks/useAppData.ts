...
export function useAppData() {
    ...
    const addStudents = useMutation({
        mutationFn: async (studentsArr: StudentInsert[]) => {
            return await Promise.all(
                studentsArr.map((s) => studentService.createStudent(s)),
            );
        },
        onSuccess: () =>
            queryClient.invalidateQueries({
    ...
    const addLessons = useMutation({
        mutationFn: async (lessonsArr: LessonInsert[]) => {
            return await Promise.all(
                lessonsArr.map((lesson) => lessonService.createLesson(lesson)),
            );
        },
        onSuccess: () =>
            queryClient.invalidateQueries({ queryKey: ["lessons", teacherId] }),
    ...
    const addGroup = useMutation({
        mutationFn: async ({
            studentIds,
            ...group
        }: GroupInsert & { studentIds: string[] }) => {
            const newGroup = await groupService.createGroup(group);
            if (studentIds.length > 0) {
                await Promise.all(
                    studentIds.map((studentId) =>
                        groupService.addStudentToGroup(newGroup.id, studentId),
                    ),
                );
            }
            return newGroup;
    ...
    const updateGroup = useMutation({
        mutationFn: async ({
            ...
            studentIds,
            ...updates
        }: GroupUpdate & { id: string; studentIds?: string[] }) => {
