    ...
    <aside className="fixed left-0 top-0 bottom-0 w-60 bg-sidebar flex flex-col z-50">
      ...
      <div className="px-5 py-5 border-b border-white/[0.06]">
        <div className="flex items-center gap-0">
          <span className="text-2xl font-bold text-white tracking-tight">less</span>
          <span className="text-2xl font-bold text-sidebar-primary tracking-tight">n</span>
        </div>
      </div>
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <item.icon className={`h-[18px] w-[18px] ${isActive ? 'text-sidebar-primary' : 'opacity-60'}`} />
                {item.label}
              </>
            )}
          </NavLink>
        ...
        <div className="px-3.5 py-3 border-t border-white/[0.06]">
          <div className="flex items-center gap-2.5 mb-2.5">
            ...
            <div className="overflow-hidden">
              <p className="text-[13px] font-semibold text-white truncate">{teacher.name}</p>
              <p className="text-[11px] text-white/35">{teacher.subject}</p>
            </div>
          </div>
          <button
            onClick={signOut}
